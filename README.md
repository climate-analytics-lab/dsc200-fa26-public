...
28 class meetings, Monday/Wednesday/Friday from Fri Sep 25 (Week 0) to Fri Dec 4 (Week 10): 24 teaching sessions including a midterm, one project clinic, and three sessions of project presentations in the last week of classes. No class on Fri Oct 16, Veterans Day (Wed Nov 11) or Thanksgiving (Thu–Fri Nov 26–27). The final exam is in finals week, in the registrar-assigned slot.
...
| # | Date | Topic | Notes |
|---|---|---|---|
| S8–S9 | Mon Oct 12 | **LLM foundations & anatomy of a harness.** Tokenization, context windows, sampling, knowledge cutoffs, tool/function calling, and their behavioral consequences (hallucinated APIs, context placement, caching and cost). The agentic loop: model + system prompt + tools + permissions + context management, and why the harness is as much of the product as the model. A real session traced from prompt to diff. | Remote (Zoom link on Canvas) |
| S10 | Wed Oct 14 | **The agentic workflow.** Requirements → spec → plan → implement → verify. Case study: the same task run from a vague and a precise spec. Introduces the spec template for project M1. | Remote (Zoom link on Canvas) |
| — | Fri Oct 16 | *No class.* | **Cancelled** |
...
| S18 | Wed Nov 4 | **Binary search trees & kd-trees.** BST property, search cost ∝ height, traversals, insertion and removal (traced by hand); degeneracy on sorted input and why balanced trees and B-tree indexes exist; binary search on sorted arrays as the array analogue; kd-trees for spatial nearest-neighbour search. | Remote (Zoom link on Canvas) |
| S19 | Fri Nov 6 | **Heaps & sorting.** Heap property and array representation; priority queues and top-*k*; sorting algorithms compared (time, extra memory, stability); what Python/NumPy/pandas actually use; stability and reproducibility; out-of-core merge. | Remote (Zoom link on Canvas) |
